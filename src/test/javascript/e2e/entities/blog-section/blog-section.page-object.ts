...
export default class BlogSectionComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('blog-section-heading'));
...
export class BlogSectionDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('ksaasApp.blogSection.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-blogSection'));
}
