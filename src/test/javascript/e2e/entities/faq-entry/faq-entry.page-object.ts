...
export default class FaqEntryComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('faq-entry-heading'));
...
export class FaqEntryDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('ksaasApp.faqEntry.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-faqEntry'));
}
