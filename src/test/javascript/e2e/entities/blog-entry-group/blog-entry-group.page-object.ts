...
export default class BlogEntryGroupComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('blog-entry-group-heading'));
...
export class BlogEntryGroupDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('ksaasApp.blogEntryGroup.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-blogEntryGroup'));
}
